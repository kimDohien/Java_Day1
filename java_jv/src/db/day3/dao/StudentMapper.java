...
public class StudentMapper implements StudentDAO{
	...
	public void insertStudent(StudentVO std) {
		String sql = "insert into student(st_num, st_name,st_semester,st_status,st_pr_num) values(?,?,?,?,?)";
		try {
			...
			dbConnector.pstmt.setString(4, std.getSt_status());
	...
	public void updateStudent(StudentVO std) {
		String sql = "update student set st_name =? , st_semester =? , st_status =?, st_pr_num = ? where st_num =?";
		try {
			dbConnector.pstmt = dbConnector.con.prepareStatement(sql);
			dbConnector.pstmt.setString(1, std.getSt_name());
			dbConnector.pstmt.setInt(2, std.getSt_semester());
			dbConnector.pstmt.setString(3, std.getSt_status());
	...
	public StudentVO selectStudent(String st_num) {
		...
		try {
			...
			if(dbConnector.rs.next()) {
				String st_name = dbConnector.rs.getString(2);//st_name이 두번쨰 위치하고 있으니까 2라 써줌
				int st_semester = dbConnector.rs.getInt(3);
				String st_status = dbConnector.rs.getString(4);
				String st_pr_num = dbConnector.rs.getString(5);
				return new StudentVO(st_num, st_name,st_semester,st_status,st_pr_num);
