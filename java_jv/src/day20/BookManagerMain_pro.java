...
public class BookManagerMain_pro {
...
//		case 옆에 문자열 " "로 받아야함
		...
		case 1://선택메뉴 1 도서추가
			...
			BookPro book = new BookPro(title,author,publish,genre,isbn,price);
			//isbn중복 체크해서 중복되면 건너뜀 (중복체크는 equals로 오버라이딩)
			//중복 : 리스트에 book객체가 포함되었다 라는 뜻 -> 포함되어있다를 알려주는 contains활용
			if(bookList.contains(book)) {
				System.out.println("이미 등록된 ISBN번호입니다.");
				System.out.println("====================");
				continue;
			}
			//중복되지 않으면 추가
		...
		case 2:
			...
			switch(subMenu){
				...
				case 3 ://출판사 조회
					...
					//해당 단어가 있으면 결과값이 나오게
					for(BookPro tmp : bookList) {
						if(tmp.getPublish().contains(publish.trim())) {
							System.out.println(tmp);
							count ++;
						}
					}
